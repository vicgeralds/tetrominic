...
export class TetGrid {
   ...
   * @param {Uint32Array | number} [blocksOrPlayfieldHeight=24]
   */
   ...
   *
   * @return {number} number of lines cleared
   */
  updateCompletedLines (row = 1) {
   ...
   *
   * @return {number} row number > 0 when there are cleared blocks to process
   */
  updateLineClears () {
    if (this.delay > 0) {
      this.delay--
  ...
  toJSON () {
    return {
      cols: this.cols,
      delay: this.delay,
      blocks: Array.from(this.blocks).map(
        b => '0x' + b.toString(16)
      )
    }
  }
...
TetGrid.parse = function (json = new TetGrid().toJSON()) {
  if (typeof json === 'string') {
    json = JSON.parse(json)
  }
  const { cols, delay = 0, blocks } = json
  const grid = new TetGrid(cols, Uint32Array.from(blocks, Number))
  grid.delay = delay
  return grid
}
...
const isMarkedAsCleared = (/** @type {number} */ b) => !(b & LINE_CLEAR_MARK)

/**
 * @param {Uint32Array} gridBlocks
 */
