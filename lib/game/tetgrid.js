...
export class TetGrid {
  ...
  constructor (cols = 10, playfieldHeight = 24) {
    this.cols = cols

    /** rows of occupied grid cells and walls */
    this.blocks = new Uint32Array(playfieldHeight)
    if (typeof playfieldHeight === 'number') {
  ...
  copy () {
    const grid = new TetGrid(this.cols, this.blocks)
    grid.delay = this.delay
    return grid
  }
  ...
  shiftClearedBlocks (row) {
    ...
    if (isEmptyRow(gridBlocks, row)) {
      // remove cleared row
      gridBlocks.set(gridBlocks.slice(row + 1), row)
    ...
    return b ^ invertBlocks(gridBlocks, row)
  }
