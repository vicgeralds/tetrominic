...
export class Tetmino extends Piece {
  ...
  constructor (piece = 0, row = 1, col = 0, delay = 1) {
    ...
    this.piece = piece
    if (tetminoShapes[piece][1] & 0x1111) {
      this.col++
    }
  }

  /** @param {CLOCKWISE | COUNTERCLOCKWISE} n */
  rotate (n = CLOCKWISE) {
    const rotationStates = tetminoShapes[this.piece]
    const i = rotationStates.indexOf(this.shape)
    const rotated = rotationStates[(i + n) % rotationStates.length]

    if (n === COUNTERCLOCKWISE && leftHandedRotationStates.includes(rotated)) {
      return rotated << 1
    }
  ...
  /** @return {number} */
  rotateRightHanded () {
    return this.rotate(COUNTERCLOCKWISE)
  }
...
const leftHandedRotationStates = [
