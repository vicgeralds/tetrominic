...
export class TetField {
  ...
  enter (piece = new Piece(), col = 0) {
    ...
    piece.col += col
    piece.falling = Math.max(SPAWN_DELAY, this.gravity) + 1
    piece.lockDelayMove = piece.falling + piece.lockDelayStep
  ...
  isRetryingAction () {
    return this.timeout[RETRY_ACTION] > this.timeout[this.lastAction]
  }

  makeMove (action = this.lastAction) {
    if (this.timeout[action] === 0 && this.piece.control(this.grid, action)) {
      if (!this.isRetryingAction() || !this.timeout[this.lastAction]) {
